...
import { AiDifficulty, AiPerformanceSummary, StatsService } from '../../src/services/StatsService';
...
export default function HomeScreen() {
  ...
  const [aiPerformance, setAiPerformance] = useState<AiPerformanceSummary | null>(null);

  const theme = getThemeColors(settings.darkMode);
  const s = useMemo(() => createStyles(theme, settings.darkMode), [theme, settings.darkMode]);
  ...
  const selectedBucket = aiPerformance?.by_difficulty[difficulty.toUpperCase() as AiDifficulty];
  ...
  const averageDuration =
    selectedBucket && selectedBucket.total_games > 0
      ? Math.round(selectedBucket.total_duration_seconds / selectedBucket.total_games)
      : 0;

  const difficultyLabel = difficulty.toUpperCase();

  return (
    <SafeAreaView edges={['top', 'left', 'right']} style={[s.container, { backgroundColor: theme.background }]}> 
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scrollContent}>
        <View style={s.header}>
          ...
          <TouchableOpacity onPress={() => router.push('/(tabs)/me' as never)} activeOpacity={0.7}>
            {profile?.avatar_url ? (
              <ExpoImage source={{ uri: profile.avatar_url }} style={s.profileImage} contentFit="cover" />
            ) : (
          ...
          <View style={s.statsGrid}>
            ...
            <View style={s.statCell}>
              ...
              <View style={s.statBarTrack}>
                <View style={[s.statBarFill, { width: `${Math.max(0, Math.min(winRate, 100))}%` }]} />
              </View>
              <Text style={s.statSub}>{`AT ${difficultyLabel}`}</Text>
            </View>
            <View style={s.statCell}>
...
const createStyles = (theme: ReturnType<typeof getThemeColors>, darkMode: boolean) =>
  StyleSheet.create({
  container: { flex: 1, backgroundColor: theme.background },
  scrollContent: { paddingHorizontal: 20 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 4, paddingBottom: 4 },
  logoRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  gridIcon: { width: 24, height: 24, flexDirection: 'row', flexWrap: 'wrap', gap: 2 },
  gridDot: { width: 10, height: 10, borderRadius: 1.5, backgroundColor: theme.accent },
  logoText: { color: theme.textPrimary, fontSize: 18, fontFamily: 'Inter_800ExtraBold', fontWeight: '800', letterSpacing: 3 },
  profileImage: { width: 38, height: 38, borderRadius: 19, borderWidth: 1, borderColor: theme.border },
  profileFallback: {
    width: 38,
    height: 38,
    borderRadius: 19,
  ...
  heroLabel: { color: theme.accent, fontSize: 11, fontFamily: 'Inter_700Bold', fontWeight: '700', letterSpacing: 2 },
  heroTitle: { color: theme.textPrimary, fontSize: 38, fontFamily: 'Inter_800ExtraBold', fontWeight: '800', lineHeight: 44, marginTop: 8 },
  startBtn: { backgroundColor: theme.accent, borderRadius: 20, paddingVertical: 22, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, marginTop: 24 },
  startBtnText: { color: theme.background, fontSize: 16, fontFamily: 'Inter_800ExtraBold', fontWeight: '800', letterSpacing: 1 },
  card: { backgroundColor: theme.elevated, borderRadius: 20, padding: 20, marginTop: 18 },
  sectionTitle: { color: theme.accent, fontSize: 12, fontFamily: 'Inter_700Bold', fontWeight: '700', letterSpacing: 2 },
  difficultyRow: {
    flexDirection: 'row',
  ...
  difficultyText: { color: theme.accent, fontSize: 11, fontFamily: 'Inter_700Bold', fontWeight: '700', letterSpacing: 1.2 },
  ...
  statLabel: { color: theme.textSecondary, fontSize: 10, fontFamily: 'Inter_700Bold', fontWeight: '700', letterSpacing: 1.5 },
  statValue: { color: theme.textPrimary, fontSize: 34, fontFamily: 'Inter_800ExtraBold', fontWeight: '800', marginTop: 2, lineHeight: 40 },
  statSub: { color: theme.textSecondary, fontSize: 11, fontFamily: 'Inter_600SemiBold', fontWeight: '600', marginTop: 4, letterSpacing: 0.5 },
  statBarTrack: {
    width: 52,
    height: 4,
    borderRadius: 2,
  ...
  dividerLabel: { color: theme.accent, fontSize: 10, fontFamily: 'Inter_700Bold', fontWeight: '700', letterSpacing: 2, marginTop: 32, marginBottom: 4 },
  trainingCard: { backgroundColor: theme.elevated, borderRadius: 20, overflow: 'hidden', marginTop: 12 },
  trainingContent: { padding: 20 },
  eliteLabel: { color: theme.accent, fontSize: 11, fontFamily: 'Inter_700Bold', fontWeight: '700', letterSpacing: 2 },
  trainingTitle: { color: theme.textPrimary, fontSize: 22, fontFamily: 'Inter_800ExtraBold', fontWeight: '800', lineHeight: 28, marginTop: 8 },
  trainingDesc: { color: theme.textSecondary, fontSize: 13, fontFamily: 'Inter_400Regular', lineHeight: 19, marginTop: 8 },
  readLink: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 14 },
  readLinkText: { color: theme.accent, fontSize: 15, fontFamily: 'Inter_700Bold', fontWeight: '700' },
  quickRow: { flexDirection: 'row', gap: 12, marginTop: 12 },
  quickCard: { flex: 1, backgroundColor: theme.elevated, borderRadius: 14, padding: 18, alignItems: 'center', gap: 8 },
  quickTitle: { color: theme.textPrimary, fontSize: 13, fontFamily: 'Inter_700Bold', fontWeight: '700' },
});
